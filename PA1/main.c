...
int main(void) {
	...
	force = mass * acceleration; // calculates the force using mass and acceleration
	...
	volume_cylinder = PI * radius * radius * height; // calculates the volume of the cylinder using the PI constant, radius, and height
	printf("volume_cylinder = PI * radius^2 * height =  %f * %f^2 * %f = %f\n\n", PI, radius, height, volume_cylinder); // outputs the volume of the cylinder
	...
	encoded_character = offset + (plaintext_character - 'a') + 'A'; // calculates the encoded character using the offset and plain text character, along with the 'a' and 'A' characters
	...
	gravity = (G * mass1 * mass2) / (distance * distance); // calculates gravity
	...
	float celsius = 0, fahrenheit = 0; // creates celsius and fahrenheit variables as floats with values 0.0

	printf("Please input the Fahrenheit value (floating-point value):\n");
	scanf("%f", &fahrenheit); // takes user input for fahrenheit

	celsius = (fahrenheit - 32) / ((float) 9 / (float) 5); // calculates celsius
	printf("celsius = (fahrenheit - 32) / (9 / 5) = (%f - 32) / (9 / 5) = %f\n\n", fahrenheit, celsius); // outputs celsius value
	...
	float distance1, x1, x2, y1, y2; // creates distance1, x1, x2, y1, and y2 variables as floats with no value
	...
	distance1 = sqrt(pow(x1 - x2, 2) + pow(y1 - y2, 2)); // calculates distance
	printf("distance = square root of ((x1 - x2)^2 + (y1 - y2)^2) = square root of ((%f - %f)^2 + (%f - %f)^2) = %f\n\n", x1, x2, y1, y2, distance1); // outputs value for distance
	...
	float y, z, x; // creates y, z, and x variables as floats with no value
	int a; // creates int variable a with no value
	...
	y = ((float)89 / (float)27) - (z * x) + (a / (a % 2)); // calculates y using z, x, and a
